...
public class BookStoreTest {
	...
	public void rateBookExceptionGeneric(Set<BookRating> bookRatingList, List<StockBook> listBooks) {
		boolean exceptionThrown = false;
		try {
		...
		try {
			List<StockBook>currentList = storeManager.getBooks();
			//We really can't trust this, but it was in the handout
			assertTrue(currentList.equals(listBooks)); 
		} catch (BookStoreException e) {
			e.printStackTrace();
			fail();
	...
	public void testRateBook() {
		...
		rateBookExceptionGeneric(bookRatingList, listBooks);
		
		Random r = new Random();
		//Just to be sure
		//Check that negative ratings will not be accepted
		
		//for(int i = 0; i < 1000; i++) {
		int i = 0;
		while(i < 1000) {
			int n = -1 * (r.nextInt(999) + 1);
			bookRatingList = new HashSet<BookRating>();
			bookRatingList.add(new BookRating(testISBN, rating));
			bookRatingList.add(new BookRating(testISBN, n));
			rateBookExceptionGeneric(bookRatingList, listBooks);
			if(localTest) {
				i++;
			} else {
	...
	public void testGetTopRatedBooks() {
		...
		exceptionThrown = false;
		Random r = new Random();
		//Check that too large inputs cause error
		int i = 0;
		while(i < 1000) {
			exceptionThrown = false;
			try {//this bugger pissed me off
				int n = listBooks.size() + 1 + r.nextInt(1000);
				client.getTopRatedBooks(n);
			} catch (BookStoreException e) {
				exceptionThrown = true;
			}
			assertTrue(exceptionThrown);
		...
		while(i < 1000) {
			exceptionThrown = false;
			try {
				client.getTopRatedBooks((r.nextInt()+1) * -1);
			} catch (BookStoreException e) {
				exceptionThrown = true;
			}
			assertTrue(exceptionThrown);
